...
func (p *PermissionController) list(ctx http.Context) *renderers.Page {
	HeaderToolbar := []any{
		...
		gamis.AjaxAction().
			Label(tools.AdminLang(ctx, "admin_permission.auto_generate")).
			Level("primary").
			Api(map[string]any{"url": tools.GetAdmin("system/_admin_permissions_auto_generate"), "method": "post"}).
			Set("confirmText", tools.AdminLang(ctx, "admin_permission.auto_generate_confirm")).
			Set("reload", "permission-crud"),
	}
	HeaderToolbar = append(HeaderToolbar, p.BaseHeaderToolBar()...)

	crud := p.BaseCRUD(ctx).Set("id", "permission-crud").HeaderToolbar(HeaderToolbar).Filter(
...
func (p *PermissionController) AutoGenerate(ctx http.Context) http.Response {
	return p.ControllerImpl.Service.AutoGenerate(ctx)
}
